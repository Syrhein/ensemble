...
@WebServlet("/postPopup")
public class PostPopupServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // DAO를 통해 작성한 게시글 목록 가져오기
        BoardDAO dao = new BoardDAO();
        List<BoardVO> postList = dao.getPostList(userId);

        // 게시글 목록을 JSP에 전달
        request.setAttribute("postList", postList);
        RequestDispatcher dispatcher = request.getRequestDispatcher("/postPopup.jsp");
